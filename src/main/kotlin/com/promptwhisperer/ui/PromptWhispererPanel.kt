...
class PromptWhispererPanel(private val project: Project) {
    ...
    private fun wireTroubleshootingActions() {
        tsAnalyseButton.addActionListener {
            ...
            if (cmd.isEmpty()) {
                log("[TS] ⚠  Enter the failed command.")
                return@addActionListener
            }
            if (errorOutput.isEmpty()) {
                log("[TS] ⚠  Paste the error output.")
                return@addActionListener
            }
    ...
    private fun currentGenerationMode(): GenerationMode =
    ...
    private fun generatePromptForMode(
        task: String,
        mode: GenerationMode,
    ): String =
        when (mode) {
    ...
    private fun String.isEffectivelyEmpty(): Boolean = isEmpty() || startsWith("(No prompt generated")
}
