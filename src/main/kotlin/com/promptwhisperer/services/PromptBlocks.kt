...
class BehaviourProfileBlock : PromptBlock {
    override fun render(context: PromptBuildContext): String {
        ...
        val profileInstructions = when (profile) {
            BehaviourProfile.BALANCED_ENGINEER -> listOf(
                "Use practical, maintainable defaults.",
                "Balance speed with code quality.",
                "Prefer incremental delivery."
            )
            BehaviourProfile.SENIOR_ARCHITECT -> listOf(
            ...
            BehaviourProfile.MINIMALIST -> listOf(
...
class ClarificationAnswersBlock : PromptBlock {
    override fun render(context: PromptBuildContext): String {
        ...
        val answerLines = if (clarifications.isEmpty()) {
...
Answer: ${question.resolvedAnswer()}"""
        ...
        val concernLines = analysis?.inferredConcerns?.takeIf { it.isNotEmpty() }
            ?.joinToString("\n") { "- inferred concern: $it" } ?: ""

        val suggestedLines = buildList {
...
class ImplementationConsiderationsBlock : PromptBlock {
    override fun render(context: PromptBuildContext): String {
        val lines = (context.inference.architectureConcerns + context.inference.operationalConcerns + context.inference.securityConcerns)
            .distinct()
...
class PlanningBalanceBlock : PromptBlock {
    override fun render(context: PromptBuildContext): String {
        ...
        val balance = when (context.config.behaviourProfile) {
            BehaviourProfile.RAPID_PROTOTYPE -> listOf(
...
class ArchitectureBlock : PromptBlock {
    override fun render(context: PromptBuildContext): String {
        ...
        val preferredApproach = context.config.clarificationQuestions
            .firstOrNull { it.id == "q_frontend_approach" }
            ?.resolvedAnswer()

        val preferenceLine = if (preferredApproach.isNullOrBlank() || preferredApproach == "Not specified") {
...
class SecurityBlock : PromptBlock {
    override fun render(context: PromptBuildContext): String {
        ...
        val lines = if (guardrails.isEmpty()) {
            listOf(
...
class TestingBlock : PromptBlock {
    override fun render(context: PromptBuildContext): String {
        ...
        val modeExtras = if (context.config.promptMode == PromptMode.TEST_GENERATION) {
...
class DocumentationBlock : PromptBlock {
    override fun render(context: PromptBuildContext): String {
        val includeDocs = context.config.enabledGuardrails.any { it.id == "cq_require_docs" } ||
...
class OperationalBlock : PromptBlock {
    override fun render(context: PromptBuildContext): String {
        ...
        val lines = if (operational.isEmpty()) {
            listOf(
...
class OutputExpectationsBlock : PromptBlock {
    override fun render(context: PromptBuildContext): String {
        val depthExpectations = when (context.config.promptDepth) {
            PromptDepth.MINIMAL -> listOf(
